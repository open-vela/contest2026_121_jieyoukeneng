...
LCD 主界面只显示日常功能：`事件`、`历史`、`测试`。设备测试、播放提示音和家庭称呼录入均不放在主界面。

| 页面 | 按钮1 | 按钮2 | 按钮3 | 按钮4 |
|---|---|---|---|---|
| 主界面 | 事件 | 历史 | 测试 | 隐藏 | 隐藏 |
| 事件 | 没事了 | 误报 | 稍后提醒 | 返回主界面 | 隐藏 |
| 历史 | 上一页 | 返回主界面 | 下一页 | 测试 | 隐藏 |
| 测试 | 麦克风采集测试 | 播放提示音 | 家庭称呼录入 | 网络测试 | 返回主界面 |
| 更多测试 | 网络测试 | 返回测试页 | 无操作 | 无操作 | 返回主界面 |
| 录入向导 | 上一步 | 下一项/采集 | 确认/保存 | 取消 | 隐藏 |

实体按键仍按 `SW2=翻页`、`SW3=已处理/向导上一步`、`SW4=误报`、`SW5=稍后提醒`、`SW6=确认/进入录入`，长按 `SW6` 对应返回。进入事件、历史或测试页后，按返回按钮或长按 `SW6` 即可回到上一层。

| # | 操作 | 预期结果 | 判定 |
|---|---|---|---|
| A9.1 | `velaguard ui home` | 监测首页：守护状态、时间、最近事件；不显示测试和录入入口 | ☐ |
| A9.2 | `velaguard ui event` | 事件确认页：最高优先事件 + 倒计时 + 三种确认 + 排队事件 | ☐ |
| A9.3 | `velaguard ui history` | 历史记录页 | ☐ |
| A9.4 | `velaguard key page` 连按 5 次 | 事件、历史、测试、更多测试、首页循环切换 | ☐ |
| A9.5 | 测试页按录入按钮 / `velaguard key enter` | 进入录入向导 `1/4 选择类别`，且提示「守护已暂停」 | ☐ |
...
真机测试约需 **2 个半天**，按依赖关系排序（不要跳序）：

| 半天 | 内容 | 原因 |
|---|---|---|
| 第 1 个半天 | B0 → B1 → B2 → B3 → B4 → B5 | 硬件链路不通时后面全白做；B2 结论决定放音方案 |
| 第 2 个半天 | B6（2 小时稳定性可挂后台）→ **B8 录入 → B7 触发** → B9 → B10 | **B8 必须先于 B7**：B7 的「姓名/求救词」一行依赖录入模板 |

### 测试道具清单（出发前备齐）

- ☐ 带测试键的烟雾报警器（或手机装好报警音样本，见 B7.0）
- ☐ 手机/蓝牙音箱（播放公开数据集样本，做可复现回放）
- ☐ 卷尺或提前量好的 1m / 3m 距离标记（贴地胶带）
- ☐ 纸箱、木板（撞击拟音，**不要用真玻璃**）
- ☐ 打印本手册 B7/B8 的记录表，两人一组：一人放声、一人记录
- ☐ 串口线 + 备用 TF 卡（拷 wav 样本用）
...
### B7.0 前置：公开样本回放预检（省现场时间的关键一步）

活体测试前，先用**训练集之外的真实公开样本**走一遍麦克风链路，把
「模型不行」和「现场操作不行」两类问题分开：

```bash
# 宿主机：从各类随机挑 5 条已转换样本（16kHz/16bit，datasets/ 下现成）
for c in alarm_beep water_flow impact moan scream; do
  ls datasets/$c/*.wav | shuf | head -5   # 拷这些文件到 TF 卡 /wav/$c/
done
```

```sh
# 板端：直接喂文件（走除麦克风外的完整识别链路）
velaguard feed /data/wav/alarm_beep/xxx.wav
velaguard log 1
# 然后用手机在 1m 处播放同一条样本，对比麦克风链路
```

| # | 检查点 | 判定 |
|---|---|---|
| B7.0a | `feed` 与「手机 1m 播放同一条」结论一致（都命中或都不中） | ☐ |
| B7.0b | 若 feed 命中而播放不中：问题在麦克风增益/距离，先调好再进 B7.1 | ☐ |
| B7.0c | 每类 5 条中 feed 命中数记录下来，作为该类的**链路上限参考** | ☐ |

### B7.1 分类触发（每类 ≥ 20 次，成功率 ≥ 80%）

> **先看当前模型的真实水平**（`model/reports/metrics.md`，2026-07-31）：
> alarm/water/scream 三类分类器 F1 0.54–0.64，可按下表直接测；
> **impact 精确率仍低（F1 0.27）**，预期需要更近距离、更大力度才能稳定命中；
> **呻吟与呼救主要靠状态机的重复确认与 B8 的个性化模板兜底**，
> 单窗口分类器对这两类还不可靠——这是已知数据缺口（FSD50K 与自采未完成），
> 测试不达标时记录数字即可，不必反复重试。

| 声景 | 触发方式与要点 | 次数 | 成功 | 成功率 | 判定 |
|---|---|---|---|---|---|
| 烟雾报警 | 真实烟感测试键，1–3m；需**连续响 ≥ 5 秒**（状态机规则，短促单响不触发是预期行为） | 20 | | | ☐ ≥80% |
| 持续水流 | 真实开水龙头/淋浴，中大水量；需**持续 ≥ 10 秒**才到提醒级 | 20 | | | ☐ ≥80% |
| 破碎撞击 | 1m 内摔纸箱、用力敲木板（**不要用真玻璃**）；力度要大，轻敲是已知盲区 | 20 | | | ☐ ≥80% |
| 呻吟/痛苦叫声 | 成员拟音，1m，明显痛苦强度，持续 2–3 秒；轻声呻吟命中率低是已知现状 | 20 | | | ☐ 记录实测值 |
| 姓名/求救词 | 成员真实呼喊（**必须先完成 B8 录入**），急促重复喊 | 20 | | | ☐ ≥80% |

每次触发后 `velaguard log 1` 确认事件类型与等级；每类换 2–3 个不同
距离/角度，不要 20 次全在同一位置。

### B7.6 误报测试（≥ 30 分钟）

按下表逐场景播放/制造声音，每场景 ≥ 5 分钟，全程 `velaguard log` 观察：

| 场景 | 时长 | 提醒级误报 | 警告级及以上误报 | 备注（类型/当时环境） |
|---|---|---|---|---|
| 电视剧（带争吵/哭喊桥段最佳） | 10 min | | | |
| 音乐（人声流行） | 5 min | | | |
| 正常交谈 2 人 | 5 min | | | |
| **平静喊名字** ×10 次 | — | | | |
| 做饭（炒菜/碗碟/抽油烟机） | 10 min | | | |

| # | 检查点 | 判定 |
|---|---|---|
| B7.6a | 30 分钟内高风险误报（警告级及以上）**≤ 1 次** | ☐ |
| B7.6b | 平静喊名字**不产生远程通知**（可以有提醒级） | ☐ |
| B7.6c | 上表每格都有记录（0 也要写 0），整表拍照进验收报告 | ☐ |

> 电视剧里的哭喊桥段是当前模型最可能误报的输入（voice 模型 none→scream
> 混淆是已知短板）。如果误报集中在这里，优先调 `velaguard_main.c` 的
> distress 置信度阈值而不是重训。
...
| B5 联网与时间 | 4 | | | |
| B6 性能稳定性 | 6 | | | |
| B7 真实触发 + 误报 | 11 | | | 核心验收；先做 B7.0 回放预检 |
| B8 个性化录入 | 13 | | | 核心验收 |
| B9 通路互斥 | 4 | | | |
| B10 异常降级 | 6 | | | |
| **合计** | **67** | | | |
